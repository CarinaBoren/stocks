...
export class StockService {
    ...
    getAllStocks(): Observable<IAllStocks> {
         ...
         return this.http.get<IAllStocks>(`https://apiservice.borsdata.se/v1/instruments?authKey=${this.authKey}`)
         .pipe(
    ...
    getKPI(): Observable<IAllKPI> {
        ...
        return  this.http.get<IAllKPI>(`https://apiservice.borsdata.se/v1/instruments/kpis/318/200day/diff?authKey=${this.authKey}`)
        .pipe(
           // tap(data => console.log('All: ' + JSON.stringify(data))),
    ...
    getStockById(stockId: number): Observable<IStock> {
        if (this.stocks) {
            ...
            if (foundItem) {
                ...
                foundItem.sector = this.sectors.sectors.find(item => item.id === foundItem.sectorId);
                return of(foundItem);
            }
        } else {
